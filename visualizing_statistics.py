...
def delay_time():
    ...
    ax_dt.set_title('Moving average passenger delay over 10 minutes', fontsize=16)
    ax_dt.set_xlabel('Time [hours]', fontsize=14)
    ax_dt.set_ylabel('Delay [s]', fontsize=14)
    ax_dt.legend(fontsize=12)
    ax_dt.xaxis.set_major_formatter(md.DateFormatter('%H'))

    fig.savefig('figures/new_passenger_delays_best.png', bbox_inches='tight')
...
def bunching_coef():
    ...
    ax_b.plot(np.linspace(0, 8, len(scheduled)), scheduled, linewidth=2, color='y', label='Schedule based')
    ax_b.set_title('Bunching levels, sampled every 10 min', fontsize=16)
    ax_b.set_xlabel('Time [h]', fontsize=14)
    ax_b.set_ylabel('Variance in distances between buses', fontsize=14)
    ax_b.legend(fontsize=12)

    fig.savefig('figures/new_bunching_levels_best.png', bbox_inches='tight')
...
def var_passengers():
    ...
    ax_vp.set_xlabel('Time [h]', fontsize=14)
    ax_vp.set_ylabel('Variance in passengers on buses', fontsize=14)
    ax_vp.set_title('Passenger variance', fontsize=16)
    ax_vp.legend(fontsize=12)
...
def waiting_passengers():
    ...
    ax_wp.set_title('Passengers waiting at stops', fontsize=16)
    ax_wp.set_ylabel('Number of passengers', fontsize=14)
    ax_wp.set_xlabel('Time [h]', fontsize=14)
